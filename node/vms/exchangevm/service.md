...
The [X-Chain](/learn/lux/lux-platform.md#x-chain),
Lux’s native platform for creating and trading assets, is an instance of the Lux Virtual
...
Deprecated as of [**v1.9.12**](https://github.com/luxfi/sdk/node/releases/tag/v1.9.12).

:::

:::warning
Not recommended for use on Mainnet. See warning notice in [Keystore API](/reference/node/keystore-api.md).
:::
...
more is ever created. The asset can be sent with `xvm.send`.
...
units of this asset using `xvm.mintNFT`.
...
can mint units of this asset using `xvm.mint`.
...
curl -X POST --data '{
    "jsonrpc":"2.0",
    "id"     : 1,
    "method" :"xvm.createVariableCapAsset",
    "params" :{
...
with [`xvm.importKey`](/reference/node/x-chain/api.md#xvmimportkey).
        ...
        "exportedOutputs": [
          {
            ...
            "fxID": "spdxUxVJQbX85MGxMHbKw1sHxMnSqJ3QBzDyDYEP3h6TLuxqQ",
            "output": {
              "addresses": ["X-lux1tnuesf6cqwnjw7fxjyk7lhch0vhf0v95wj5jvy"],
              "amount": 2569382395,
              "locktime": 0,
              "threshold": 1
            }
...
- `blockchainID` is the ID of the blockchain this transaction happened on. (Lux Mainnet
...
- `assetID`: The ID of the asset being transferred. (The Mainnet Lux ID is
...
`X-lux18jma8ppw3nhx5r4ap8clazz0dps7rv5ukulre5` and `X-lux1d09qn852zcy03sfc9hay2llmn9hsgnw4tp3dv6`.
...
- `to` is the address the LUX is sent to. This must be the same as the `to` argument in the
  corresponding call to the P-Chain’s `exportLUX` or C-Chain's `export`.
- `sourceChain` is the ID or alias of the chain the LUX is being imported from. To import funds
...
[`xvm.createVariableCapAsset`](/reference/node/x-chain/api.md#xvmcreatevariablecapasset).
...
[`xvm.createNFTAsset`](/reference/node/x-chain/api.md#xvmcreatenftasset).
...
- Sends `amount` units of asset with ID `assetID` to address `to`. `amount` is denominated in the
  smallest increment of the asset. For LUX this is 1 nLUX (one billionth of 1 LUX.)
...
Endpoint deprecated as of [**v1.9.12**](https://github.com/luxfi/sdk/node/releases/tag/v1.9.12).
...
import (
    ...
    "github.com/luxfi/sdk/node/api"
    "github.com/luxfi/sdk/node/pubsub"
