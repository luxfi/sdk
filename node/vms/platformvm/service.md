The P-Chain API allows clients to interact with the [P-Chain](https://build.lux.network/docs/quick-start/primary-network#p-chain), which maintains Lux’s validator set and handles blockchain creation.
...
The response in this example indicates that LUX’s supply is at most 365.865 million.
...
- `validators` can include different fields based on the subnet type (L1, PoA Nets, the Primary Network):
  ...
  - `delegators` is the list of delegators to this validator. Omitted if `subnetID` is not the Primary Network. Omitted unless `nodeIDs` specifies a single NodeID.
    ...
    - `weight` is the amount of nLUX this delegator staked.
...
Get the amount of nLUX staked by a set of addresses. The amount returned does not include staking
rewards.
  ...
  "result": {
    "tx": {
      "unsignedTx": {
        ...
        "stake": [
          {
            ...
            "fxID": "spdxUxVJQbX85MGxMHbKw1sHxMnSqJ3QBzDyDYEP3h6TLuxqQ",
            "output": {
              "addresses": ["P-lux1tnuesf6cqwnjw7fxjyk7lhch0vhf0v95wj5jvy"],
              "amount": 48944170378,
              "locktime": 0,
              "threshold": 1
            }
        ...
        "rewardsOwner": {
          "addresses": ["P-lux19zfygxaf59stehzedhxjesads0p5jdvfeedal0"],
          "locktime": 0,
          "threshold": 1
        }
      },
...
`P-lux18jma8ppw3nhx5r4ap8clazz0dps7rv5ukulre5` and `P-lux1d09qn852zcy03sfc9hay2llmn9hsgnw4tp3dv6`.
