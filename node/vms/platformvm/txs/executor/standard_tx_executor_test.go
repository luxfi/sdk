...
import (
	...
	"github.com/luxfi/sdk/node/genesis/builder"
	...
	"github.com/luxfi/sdk/node/vms/platformvm/fx/fxmock"
	...
	"github.com/luxfi/sdk/node/vms/platformvm/utxo/utxomock"
	...
	"github.com/luxfi/sdk/node/wallet/net/primary/common"

	safemath "github.com/luxfi/math"
	txfee "github.com/luxfi/sdk/node/vms/platformvm/txs/fee"
	validatorfee "github.com/luxfi/sdk/node/vms/platformvm/validators/fee"
)
	...
	tests := []test{
		...
		},
		{
			...
			newExecutor: func(ctrl *gomock.Controller) (*txs.RemoveChainValidatorTx, *standardTxExecutor) {
				...
				env.fx.EXPECT().VerifyPermission(gomock.Any(), env.unsignedTx.ChainAuth, env.tx.Creds[len(env.tx.Creds)-1], subnetOwner).Return(errTest)
...
func TestStandardExecutorConvertChainToL1Tx(t *testing.T) {
	var (
		fx = &secp256k1fx.Fx{}
	...
	tests := []struct {
		...
		},
		{
			name: "fail subnet authorization",
			updateExecutor: func(e *standardTxExecutor) error {
				e.state.SetNetOwner(subnetID, &secp256k1fx.OutputOwners{
					Threshold: 1,
					Addrs: []ids.ShortID{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			stateConversion, err := diff.GetNetToL1Conversion(subnetID)
			require.NoError(err)
			require.Equal(
				state.NetToL1Conversion{
...
func TestStandardExecutorRegisterL1ValidatorTx(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			updateExecutor: func(e *standardTxExecutor) error {
				e.state.SetNetToL1Conversion(subnetID, state.NetToL1Conversion{})
				return nil
			},
