...
func TestCaptureValidatorBalanceWithMonkeyPatch(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedBalance:  0,
			expectError:      true,
			errorContains:    "entered value has to be greater than 0 LUX",
		},
		{
			...
			expectedBalance:  0,
			expectError:      true,
			errorContains:    "validator balance must be at least 10.00 LUX",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			promptUIRunner = func(prompt promptui.Prompt) (string, error) {
				...
				if tt.mockReturn != "" && tt.mockError == nil {
					switch {
					case strings.Contains(tt.errorContains, "entered value has to be greater than 0 LUX") ||
...
func TestCaptureValidatorBalanceEdgeCases(t *testing.T) {
	...
	t.Run("prompt label preserved", func(t *testing.T) {
		expectedLabel := "Please enter the validator balance in LUX"
		promptUIRunner = func(prompt promptui.Prompt) (string, error) {
			require.Equal(t, expectedLabel, prompt.Label)
			return "25.0", nil
		}
	...
	t.Run("validator balance validation logic", func(t *testing.T) {
		...
		promptUIRunner = func(prompt promptui.Prompt) (string, error) {
			...
			require.Error(t, err1)
			require.Contains(t, err1.Error(), "entered value has to be greater than 0 LUX")

			err2 := prompt.Validate("3.0") // Below minimum
			require.Error(t, err2)
			require.Contains(t, err2.Error(), "validator balance must be at least 5.00 LUX")
