...
func (mc *mockClient) SendRequest(_ context.Context, _ string, _ interface{}, reply interface{}, _ ...rpc.Option) error {
	...
	switch p := reply.(type) {
	case *apitypes.EmptyReply:
		response := mc.response.(*apitypes.EmptyReply)
		*p = *response
	case *apiadmin.GetChainAliasesReply:
		response := mc.response.(*apiadmin.GetChainAliasesReply)
		*p = *response
	case *apiadmin.LoadVMsReply:
		response := mc.response.(*apiadmin.LoadVMsReply)
		*p = *response
	case *apiadmin.LoggerLevelReply:
		response := mc.response.(*apiadmin.LoggerLevelReply)
		*p = *response
	case *interface{}:
...
func TestStartCPUProfiler(t *testing.T) {
	for _, test := range SuccessResponseTests {
		t.Run(test.name, func(t *testing.T) {
			mockClient := Client{Requester: NewMockClient(&apitypes.EmptyReply{}, test.expectedErr)}
...
func TestGetChainAliases(t *testing.T) {
	t.Run("successful", func(t *testing.T) {
		...
		mockClient := Client{Requester: NewMockClient(&apiadmin.GetChainAliasesReply{
	...
	t.Run("failure", func(t *testing.T) {
		mockClient := Client{Requester: NewMockClient(&apiadmin.GetChainAliasesReply{}, errTest)}
...
func TestReloadInstalledVMs(t *testing.T) {
	...
	t.Run("failure", func(t *testing.T) {
		mockClient := Client{Requester: NewMockClient(&apiadmin.LoadVMsReply{}, errTest)}
...
func TestSetLoggerLevel(t *testing.T) {
	type test struct {
		...
		logLevel        string
		displayLevel    string
		serviceResponse map[string]apiadmin.LogAndDisplayLevels
		serviceErr      error
		clientErr       error
	}
	tests := []test{
		{
			...
			displayLevel: "INFO",
			serviceResponse: map[string]apiadmin.LogAndDisplayLevels{
				"Happy path": {LogLevel: "INFO", DisplayLevel: "INFO"},
			},
			serviceErr: nil,
			clientErr:  nil,
		},
	...
	tests := []test{
		{
			...
			loggerName: "foo",
			serviceResponse: map[string]apiadmin.LogAndDisplayLevels{
				"foo": {LogLevel: "INFO", DisplayLevel: "INFO"},
			},
			serviceErr: nil,
			clientErr:  nil,
		},
